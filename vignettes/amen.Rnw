...
\author{Peter D. Hoff
\thanks{Departments of Statistics and Biostatistics, University of Washington, Seattle. 
\url{http://www.stat.washington.edu/\~pdhoff/}
...
A pair of objects, individuals or nodes is called a \emph{dyad}, and 
a variable that is measured or observed on multiple dyads
...
denoted  $y_{i,j}$, 
gives the 
value of the variable for  dyad $\{i,j\}$
...
$\{a_i: i=1,\ldots, n\}$ and 
$\{b_j : j=1,\ldots, n\}$ corresponds to 
...
no column heterogeneity (all $b_j$'s equal to zero) 
...
two observations come from the same dyad. 
...
A seminal model for analyzing such 
...
<<IR90s_fit_srm,cache=TRUE,fig.keep='last',results='hide',fig.cap='Default plots generated by the {\\tt ame} command.',fig.height=6,fig.width=5,fig.align='center'>>=
fit_SRM<-ame(Y) 
@
...
the list component {\tt VC} in the fitted object.
The only regression parameter for this  SRM model is the
intercept $\mu$, which is included by default for the Gaussian SRM.
The intercept, and any other regression parameters are
stored as {\tt BETA} in the fitted object.
...
\Sexpr{round(gofstats(Y),3)[1:3]} 
and \Sexpr{round(gofstats(Y),3)[4]}, 
 given by vertical red lines in the figure.
Generally speaking,
...
The SRRM can be fit 
by specifying the
covariates in the {\tt ame} function:
...
under an ordinary linear regression model that assumes i.i.d.\ residual standard error. 
...
almost all smaller 
than those under the SRM fit. The explanation for this is that 
...
@
\caption{Comparison of two  SRRMs
in terms of the triadic dependence statistic: 
with nodal
...
term of the form  $\bl x_{r,i}^T \bl B \bl x_{c,j}$ can also 
...
less similar conclusions as those from the SRRM, 
except that the number of shared IGOs
...
In some cases, such as with the trade data, the variable of interest can be transformed 
so that the 
Gaussian 
...
can be put in some meaningful order. This includes discrete outcomes 
...
Such dyadic data, 
%Data on such relations, 
particularly data indicating 
social interactions or friendships, are often collectively called 
a \emph{social network}. For example, 
...
of a law firm, 
along with data on two other dyadic variables and several nodal 
variables. The friendship data are displayed as a graph in Figure 
\ref{fig:llaw_graph}, where the nodes are colored according to 
each lawyer's office location. 
...
Assuming the SRM for the sociomatrix $\bl Z=\{z_{i,j}\}$ of latent variables 
...
of the three statistics that represent  second-order dependence. 
...
@
\caption{Comparison of the SRM (blue) and  the SRG (green) for the 
...
Inference then proceeds by examining
the estimates of regression effects, 
random effects and covariance parameters. 
Interpretation of the multiplicative effects 
can proceed by plotting them, looking for clusters, and 
...
(as estimation of $g$ is not specifically provided), 
...
We examine this effect more fully with an  ordinal probit regression, 
...
and possibly  to rank these friends in terms of their affinities to them. 
...
all $n(n-1)$ ordered pairs of nodes, some researchers use 
...
Partially observed dyadic data on a given nodeset
can be represented by a sociomatrix in which the 
ordered pairs for which data are not observed  are distinguished 
...
the probability of a missing data value for a pair is 
independent of the model parameters and missing 
...
``with whom are you friends'' and ``which of your friends are friends 
...
Under this design, data between alters and non-alters of an ego
are missing, as are data between alters that do not share an ego.
<<>>=
egos
Ys[1:10,1:10]
@
...
@
The first and second numbers reflect ``within-sample'' 
...
of 100 egocentric samples randomly generated as previously described, we obtain 
...
egocentric samples is illustrated with histograms in 
...
sociomatrix as predictors. Additionally, 
time-varying regression parameters can be included in the 
model by constructing interactions. 
...
Similarly, 
the dyadic regressor array {\tt Xdyad} is now four-dimensional, 
...
time lag between measurements. Such a possibility can be evaluated simply 
...
$w_t$ is a binary indicator that $t$ is 
...
Most of the simplifications leading to this symmetric model are 
...
in the previous sections can be fit 
by simply specifying the option  {\tt symmetric=TRUE} in the 
 {\tt ame} command. 
...
@ 
%<<cwfitR2,cache=TRUE,fig.keep='none',results='hide'>>=
%fit_cw_R2<-ame(Y,Xd,Xn,R=2,model="ord",symmetric=TRUE,burn=1000,nscan=100000,odens=100)
%@
\begin{figure}
<<echo=FALSE>>=
plot(fit_cw_R1)
...
the number of iterations to 100,000 from the default value of 10,000. 

<<>>=
summary(fit_cw_R1) 

fit_cw_R1$L       # eigenvalue
@
...
More refined hypotheses about conflict and 
