...
\begin{figure}
<<fig.height=4.75,fig.width=4.75,fig.align='center'>>=
data(lazegalaw) 
...
Some types of dynamic relational datasets include 
...
different dyads \emph{or} different time points as independent. 
...
dynamic relational data, as it doesn't seem to allow for the 
possibility of dependence over time. 
However, 
certain types of dependence can be incorporated into this model 
via the 
time-dependent 
regression terms. Specifically, autoregressive
...
$t$. 
The graphs of this relation for each of  the seven different time points 
...
study period, and so not surprisingly, the density of the graphs
increase over time. 
...
relations within a dyad.  To summarize, our model 
...
There also appear to be strong homophily effects in 
...
It is sometimes the case that relational 
observations are \emph{symmetric} or 
\emph{undirected} by design, in that there is 
only one relational value $y_{i,j}$ for the 
...
In the former case, this representation can justified 
...
we fit an  ordinal version of a rank-2 symmetric  AME  model 
 using the{\tt ame} command:
<<cwfit,cache=TRUE,fig.keep='none',results='hide'>>=
fit_cw<-ame(Y,Xd,Xn,R=2,model="ord",symmetric=TRUE,burn=10000,nscan=100000,odens=100)
@ 
...
As shown above, 
the estimated ``eigenvalues'' {\tt fit\_cw\$L} are both positive. 
...
cooperative pairs (linked by green lines) are ``nearby'' 
on the plot, as their latent vectors are 
in similar directions. 
Conversely, 
country pairs in conflict (linked by red lines)
generally have vectors pointing in  opposite directions. 
