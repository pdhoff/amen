...
study period, and so not surpisingly, the density of the graphs
increase over time. 
...
attributes on frienship in using a probit 
...
{\tt ame} command discussed previsouly, and results can 
...
$y_{i,j,t-1}$ on $z_{i,j,t}$ varies with measurment interval, 
...
\begin{align}
 y_{i,j}  & = \beta_d^T \bl x_{i,j} + \beta_n^T ( \bl x_{i} + \bl x_{j} ) +  a_i + a_j +  \bl u_i^T \bl \Lambda  \bl u_j +  \epsilon_{i,j} ,    \\
...
by simply specifiying the option  {\tt symmetric=TRUE}. 
