...
for a class of additive and multiplicative random effects models of 
dyadic data of a variety of types, 
including 
ordinal, continuous and binary 
outcomes. The package also provides methods 
for missing, censored and fixed-rank nomination data, as well 
as longitudinal dyadic data.  This vignette illustrates the 
use of the {\tt amen} package 
via analyses of several of these different types of dyadic data. 
...
with the SRM covariance structure that recognizes the dyadic 
aspect of the data. 
...
Dyadic datasets often include variables that 
...
up to a certain number of friends, but not rank them. Such dyadic data
...
pairs $i,j$ for which we do not know the  value of 
...
country-specifc levels  of $z_{i,j}$ 
...
fitting separate models for the 
...
cooperative pairs (linked by green lines) 
 essentailly all have $u$-values that are on the same side of the origin
