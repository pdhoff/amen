...
Some relational datasets include repeated measures, that 
is multiple observations of dyadic and nodal variables 
over time. The {\tt amen} package provides a rudimentary 
method of analyzing such data, which assumes 
...
The {\tt amen} package provides a model fitting routine 
that can accomodate 
...
<<cache=TRUE,fig.keep='none',results='hide'>>= 
fit_iid<-ame_rep(Y,Xdyad[,,-(1:3),],Xnode,Xnode,model="bin",plot=FALSE)
...
<<echo=FALSE>>=
#par(mfrow=c(1,2),mar=c(3,3,1,1),mgp=c(1.75,.75,0))
#boxplot( c(fit_iid$YPM)~c(Y),xlab="Y",ylab=expression(hat(Y)),col="lightgreen")
#mtext(side=3,"IID model") 
#boxplot( c(fit_ar1$YPM)~c(Y),xlab="Y",ylab=expression(hat(Y)),col="lightgreen")
#mtext(side=3,"AR1 model")
@


<<>>=
#mean( (fit_iid$YPM-Y)^2, na.rm=TRUE )
#mean( (fit_ar1$YPM-Y)^2, na.rm=TRUE )
@
